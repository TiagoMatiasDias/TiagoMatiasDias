...
Cada partida tem um bônus (`home`/`draw`/`away`) calculado a partir da
diferença de posição na tabela entre os dois times
(`computeOutcomeDifficultyBonus`, em `packages/scoring/src/difficultyBonus.ts`).
Reproduz exatamente o exemplo do enunciado — líder x lanterna (campeonato de
20 times): favorito vencendo = +2, empate = +4, zebra (azarão vencendo) = +8.
...
parelhos geram bônus menor/nulo.

> **Fórmula é um ponto de partida, não está fechada.** O enunciado só deu um
> exemplo (2/4/8); eu criei uma fórmula linear (`maxFavoriteBonus × gap
> normalizado`, empate = 2×, zebra = 4×) que bate com esse exemplo, mas os
> multiplicadores e o valor máximo são ajustáveis
> (`computeOutcomeDifficultyBonus(... , { maxFavoriteBonus })`). Vale a pena
> revisar com dados reais do campeonato antes de travar os valores.
